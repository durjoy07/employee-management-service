...
export class EmployeeService {
  ...
  async getAllEmployee(id: number): Promise<EmployeeResponseDto[]> {
    try {
      ...
      const cachedData = await this.cacheManager.get<EmployeeResponseDto[]>(
        cachedKey
      );
      ...
      if (!employee)
        throw new InternalServerErrorException('No data found with given id');
      ...
      const result = allEmployees.map((employee) => {
        const children = allEmployees
          .filter(
            (childrenMembers) =>
              childrenMembers.position_id > employee.position_id
          )
          .map((childrenMembers) => ({
            ...childrenMembers,
      ...
      function buildChildrenStructure(children, employees) {
        return employees
          .filter(
            (childEmployees) =>
              childEmployees.position_id > children.position_id
          )
          .map((childEmployees) => ({
            ...childEmployees,
